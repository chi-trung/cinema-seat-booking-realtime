 ...
 */
app.post('/api/admin/movies', authenticateToken, adminOnly, upload.single('poster'), async (req, res) => {
...
/**
 * ENDPOINT 7: Lấy trạng thái ghế cho phim
 * Method: GET
 */
...
app.post('/api/bookings', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Tạo booking
    const bookingId = await db.createBooking(movieId, req.user.id, seats, totalPrice);
...
/**
 * ENDPOINT 9: Lấy booking của user
 * Method: GET
 * Requires: Token
 */
...
/**
 * ENDPOINT 10: Lấy tất cả booking (Admin)
 * Method: GET
 * Requires: Token + Admin role
 */
app.get('/api/admin/bookings', authenticateToken, adminOnly, async (req, res) => {
  console.log(`📡 HTTP GET /api/admin/bookings (Admin: ${req.user.username})`);
...
server.listen(PORT, () => {
  console.log('\n' + '='.repeat(70));
  console.log('🎬 CINEMA REALTIME BOOKING SYSTEM - WITH AUTHENTICATION & DATABASE');
